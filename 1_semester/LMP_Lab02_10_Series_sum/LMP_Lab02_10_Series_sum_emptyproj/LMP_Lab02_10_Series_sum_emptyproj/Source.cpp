...
bool is_correct_double(string input) {						// check for correctness
	...
	if ((isdigit(input[0]) == 0) && (input[0] != '.') && (input[0] != '-'))
		correct = false;								//if 1st char is a non-digital char exept '.' and '-', then not correct
	if (input[0] == '.')
		dotcount++;
...
int main() {
	...
	cout << "Please use real numbers " << MAX_ACCEPTED_D_LEN << " symbols long" << endl;
						...
						{	// Z
							...
							if (alpha > eps.size())
								for (int i = eps.size(); i <= alpha - 1; ++i)
								{
									...
									cout << setprecision(11) << "n: " << i << '\t' << "an: " << a[i] << '\t' << "Sn: " << sum[i] << '\t' << "epsn: " << eps[i] << endl;

								}
							else
							{
								cout << setprecision(11) << "n: " << alpha - 1 << '\t' << "an: " << a[alpha - 1] << '\t' << "Sn: " << sum[alpha - 1] << '\t' << "epsn: " << eps[alpha - 1] << endl;
							}
						}
						else
						{
